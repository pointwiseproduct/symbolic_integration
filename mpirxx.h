...
inline void __mpz_set_si_safe(mpz_ptr p, mpir_si l)
{
  if(l < 0)
  {
    __mpz_set_ui_safe(p, ~static_cast<mpir_ui>(l) + 1);
    mpz_neg(p, p);
  }
  else
...
struct __gmp_binary_plus
{
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, mpir_si l)
  {
    if (l >= 0)
      ...
      mpz_sub_ui(z, w, ~static_cast<mpir_ui>(l) + 1);
  }
  ...
  static void eval(mpf_ptr f, mpf_srcptr g, mpir_si l)
  {
    if (l >= 0)
      ...
      mpf_sub_ui(f, g, ~static_cast<mpir_ui>(l) + 1);
  }
...
struct __gmp_binary_minus
{
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, mpir_si l)
  {
    if (l >= 0)
      ...
      mpz_add_ui(z, w, ~static_cast<mpir_ui>(l) + 1);
  }
  static void eval(mpz_ptr z, mpir_si l, mpz_srcptr w)
  {
    if (l >= 0)
  ...
  static void eval(mpq_ptr q, mpq_srcptr r, mpir_si l)
  {
    if (l >= 0)
      ...
      __gmp_binary_plus::eval(q, r, ~static_cast<mpir_ui>(l) + 1);
  }
  static void eval(mpq_ptr q, mpir_si l, mpq_srcptr r)
  { eval(q, r, l); mpq_neg(q, q); }
  static void eval(mpq_ptr q, mpq_srcptr r, double d)
  ...
  static void eval(mpf_ptr f, mpf_srcptr g, mpir_si l)
  {
    if (l >= 0)
      ...
      mpf_add_ui(f, g, ~static_cast<mpir_ui>(l) + 1);
  }
  static void eval(mpf_ptr f, mpir_si l, mpf_srcptr g)
  {
    if (l >= 0)
...
__gmp_binary_plus::eval(mpq_ptr q, mpq_srcptr r, mpir_si l)
{
  if (l >= 0)
    ...
    __gmp_binary_minus::eval(q, r, ~static_cast<mpir_ui>(l) + 1);
}
...
// gcc-3.3 doesn't have __builtin_ctzl. Don't bother optimizing for old gcc.
  ...
  static void eval(mpz_ptr z, mpz_srcptr w, mpir_si l)
  {
    if (__GMPXX_CONSTANT(l))
    {
      ...
      {
        eval(z, w, ~static_cast<mpir_ui>(l) + 1);
	mpz_neg(z, z);
      }
    }
    else
  ...
  static void eval(mpq_ptr q, mpq_srcptr r, mpir_si l)
  {
    if (__GMPXX_CONSTANT(l))
    {
      ...
      {
        eval(q, r, ~static_cast<mpir_ui>(l) + 1);
	mpq_neg(q, q);
      }
    }
    else
  ...
  static void eval(mpf_ptr f, mpf_srcptr g, mpir_si l)
  {
    if (l >= 0)
      ...
      {
	mpf_mul_ui(f, g, ~static_cast<mpir_ui>(l) + 1);
	mpf_neg(f, f);
      }
  }
  static void eval(mpf_ptr f, mpir_si l, mpf_srcptr g)
...
struct __gmp_binary_divides
{
  ...
  static void eval(mpf_ptr f, mpf_srcptr g, mpir_si l)
  {
    if (l >= 0)
      ...
      {
	mpf_div_ui(f, g, ~static_cast<mpir_ui>(l) + 1);
	mpf_neg(f, f);
      }
  }
  static void eval(mpf_ptr f, mpir_si l, mpf_srcptr g)
  {
    if (l >= 0)
      ...
      {
	mpf_ui_div(f, ~static_cast<mpir_ui>(l) + 1, g);
	mpf_neg(f, f);
      }
  }
  static void eval(mpf_ptr f, mpf_srcptr g, double d)
...
public:
  ...
  std::string get_str() const{
      if(mp->_mp_size == 0){ return "0"; }
      mp_exp_t e;
      std::string r = get_str(e), s;
      if(e == 0){
          s = "0.";
          if(mp->_mp_size >= 0){
              return r.insert(0, s);
          ...
          s = ".";
          if(r.size() == (e + (mp->_mp_size < 0 ? 1 : 0))){ s = ""; }
          if(mp->_mp_size >= 0){
  ...
  //追記された関数
  //mpfフォーマットの指数部を得る
  mp_exp_t exp() const{ return mp->_mp_exp; }

  //追記された関数
  //mpfフォーマットの精度を得る
  mp_exp_t prec() const{ return mp->_mp_prec; }

  //追記された関数
  //mpf版dexp(k) 2^k倍する
  void dexp(mp_exp_t k_div, mp_exp_t k_mod){
      mp->_mp_exp += k_div;
      unsigned int n = k_mod;
      const int size = mp->_mp_size < 0 ? -mp->_mp_size : mp->_mp_size;
      mp_limb_t *r_d = mp->_mp_d;
      const int m = (sizeof(mp_limb_t) * 8) - nlz(r_d[size - 1] >> ((sizeof(mp_limb_t) * 8) - n));
      mp_limb_t last = r_d[size - 1] >> ((sizeof(mp_limb_t) * 8) - n);
      for(int i = size - 1; i > 0; --i){
          r_d[i] = (r_d[i] << n) | (r_d[i - 1] >> ((sizeof(mp_limb_t) * 8) - n));
      }
      r_d[0] <<= n;
      if(last > 0){
          ++r_d;
  ...
  //追記された関数
  //mpfフォーマットで範囲が0かどうか
  bool zero_range(unsigned int n) const{
      for(unsigned int i = 0; i < n; ++i){
          if(mp->_mp_d[mp->_mp_size - i - 1]){ return false; }
      }
      return true;
  }
  ...
  //追記された関数
  //仮数部を得る
  __gmp_expr r2_significand() const{
      const __gmp_expr &a(*this);
      const mp_limb_t *a_d = a.mp->_mp_d;
      const int a_size = a.mp->_mp_size < 0 ? -a.mp->_mp_size : a.mp->_mp_size;
      int n = nlz(a_d[a_size - 1]);
      __gmp_expr r(*this);
      r.mp->_mp_exp = 0;
      mp_limb_t *r_d = r.mp->_mp_d;
  ...
  //追記された関数
  //指数部を得る
  mp_exp_t r2_exp() const{
      const __gmp_expr &a(*this);
      const mp_limb_t *a_d = a.mp->_mp_d;
      const mp_exp_t a_e = a.mp->_mp_exp;
      const int a_size = a.mp->_mp_size < 0 ? -a.mp->_mp_size : a.mp->_mp_size;
      return (a_e - 1) * sizeof(mp_limb_t) * 8 + sizeof(mp_limb_t) * 8 - nlz(a_d[a_size - 1]);
  }
...
private:
    // 追加された関数
    static int nlz(mp_limb_t x){
        auto make_half_mask = [](int bit){
            mp_limb_t mask = 0;
            mask = ~mask;
            mp_limb_t one = static_cast<mp_limb_t>(1) << (sizeof(mp_limb_t) * 8 - 1);
            for(int i = 0; i < bit; ++i){
                mask &= ~(one >> i);
            }
            return mask;
        ...
        if(x == 0){ return sizeof(mp_limb_t) * 8; }
        int n = 0;
        for(int i = sizeof(mp_limb_t) * 8 / 2; i > 0; i >>= 1){
            if(x <= make_half_mask(i)){
                n = n + i;
                x <<= i;
            }
        }
